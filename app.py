...
def webhook():
    ...
    if intent == "UploadImageIntent":
        ...
        if TEMP_INGREDIENTS:
            session = req.get("session", "")  # Get session ID for full context path
        ...
        # Get context ingredients if needed

        for context in req.get("queryResult", {}).get("outputContexts", []):

            if "ingredient-followup" in context["name"]:

                context_ingredients = context.get("parameters", {}).get("ingredients", [])

                if context_ingredients:
                    TEMP_INGREDIENTS = context_ingredients  # Load previous ingredients
